...
- Custom hooks: `useCamelCase` for both the symbol and the file (`useDebounce.ts` exports `useDebounce`) — see CLAUDE.md §Naming Conventions for project-specific convention
...
- External store: **Zustand** for global UI state (see SSOT.md §3.5) — no server state in Zustand
...
This project uses Tailwind CSS v4 — no per-component CSS files. Single-file components in `components/sections/` or `components/ui/`. See SSOT.md §4.3 for the canonical folder structure.

Inline single-file components are the standard for this project.
